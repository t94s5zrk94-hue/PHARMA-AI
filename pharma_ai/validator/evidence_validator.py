...
class EvidenceValidator:
    ...
    def load_master(self):
        ...
        self.logger.info("Loading Evidence Master...")
    ...
    def validate_duplicate_ids(self):
        ...
        duplicates = self.df["Evidence_ID"].duplicated().sum()

        self.statistics["duplicate_ids"] = int(duplicates)

        if duplicates > 0:
            raise ValueError(
                f"Duplicate Evidence_ID found: {duplicates}"
            )

        self.logger.info(
            "Evidence_ID validation passed."
        )
    ...
    def validate_duplicate_pairs(self):
        ...
        if duplicates > 0:
            raise ValueError(
                f"Duplicate Evidence pair found: {duplicates}"
            )

        self.logger.info(
            "Evidence pair validation passed."
        )
